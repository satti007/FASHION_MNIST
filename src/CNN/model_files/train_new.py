...
def conv2d(input_layer,filters,ksize,stride,padding,scope,isTraining):
	with tf.variable_scope(scope) as scope:
		return tf.contrib.layers.convolution2d(inputs=input_layer,num_outputs=filters,kernel_size=ksize,stride=stride,
		padding=padding,scope=scope,normalizer_fn=tf.contrib.layers.batch_norm,normalizer_params={'is_training':isTraining,'updates_collections':None,'scale':True})
...
def FC(input_layer,neurons,scope,isTraining):
	with tf.variable_scope(scope) as scope:
...
def softmax(input_layer,neurons,scope,isTraining):
	with tf.variable_scope(scope) as scope:
		return tf.contrib.layers.fully_connected(inputs=input_layer,num_outputs=neurons,activation_fn=None,scope=scope,
		normalizer_fn=tf.contrib.layers.batch_norm,normalizer_params={'is_training':isTraining,'updates_collections':None,'scale':True})

def model(x,isTraining,keep_prob1,keep_prob2):
	with tf.contrib.slim.arg_scope([tf.contrib.slim.model_variable, tf.contrib.slim.variable]):
		
		# TODO: convlayer: input = 28,28,1  output = 28,28,32
		layer_32 = conv2d(x,32,3,1,'SAME','conv32',isTraining)
		
		# TODO: convlayer: input = 28,28,32  output = 28,28,64
		layer_1  = conv2d(layer_32,64,3,1,'SAME','conv1',isTraining)
		
		# TODO: poollayer: input = 28,28,64  output = 14,14,64
		pool_1   = max_pool2d(layer_1,2,2)
		
		# TODO: convlayer: input = 14,14,64 output = 14,14,128
		layer_2  = conv2d(pool_1,128,3,1,'SAME','conv2',isTraining)        
		
		# TODO: poollayer: input = 14,14,128  output = 7,7,128
		pool_2   = max_pool2d(layer_2,2,2)
		
		# TODO: convlayer: input = 7,7,128  output = 7,7,256
		layer_3  = conv2d(pool_2,256,3,1,'SAME','conv3',isTraining)
		
		# TODO: convlayer: input = 7,7,256  output = 7,7,256
		layer_4  = conv2d(layer_3,256,3,1,'SAME','conv4',isTraining)
		...
		layer_f_d = tf.nn.dropout(layer_f,keep_prob=keep_prob1) # dropout layer
		
		# TODO: FC layer: input = 2304 output = 2048
		layer_5   = FC(layer_f,2048,'fc5',isTraining)
		layer_5_d = tf.nn.dropout(layer_5,keep_prob=keep_prob1) # dropout layer
		
		# TODO: FC layer: input = 2048  output = 1024
		layer_6   = FC(layer_5,1024,'fc6',isTraining) 
		layer_6_d = tf.nn.dropout(layer_6,keep_prob=keep_prob1) # dropout layer
		
		# TODO: FC layer: input = 1024  output = 10
		logits    = softmax(layer_6,10,'fc7',isTraining)
		logits_d  = tf.nn.dropout(logits,keep_prob=keep_prob2) # dropout layer
		
		# TODO: softmax layer: input = 10  output = 10
		y = tf.nn.softmax(logits_d,name='output_node')
...
lr,batch_size,init = get_arguments() # Parsing the argumnets
train,valid,test,save_dir                   = '../data/train.csv','../data/val.csv','../data/test.csv','weights/'
...
logits,y      = model(x,isTraining,keep_prob1,keep_prob2)
...
def dotest(epoch,test_X):
	...
	for s in range(steps):
		...
		pred   = sess.run(y,feed_dict={x:xs,isTraining:False,keep_prob1:1,keep_prob2:1})
...
def show_loss_acc(isTrain,train_X,train_y,valid_X,valid_y,epoch,step):
	...
	for i in range(1, S+1):
		...
		if isTrain:
			xs,ys = get_aug_data(xs,ys) # get augmented batch data
		loss += sess.run(cross_entropy,feed_dict = {x:xs,y_:ys,isTraining:True,keep_prob1:1,keep_prob2:1})
		pred  = sess.run(y,feed_dict = {x:xs,isTraining:isTrain,keep_prob1:1,keep_prob2:1})
		if isTrain:
			acc  += accuracy(pred.reshape(4*Vbatch_size,10),ys)
		else:
...
max_epochs, show, prob1, prob2 = 2, 50, 0.6, 0.9
...
def train(train_X,train_y,valid_X,valid_y,test_X,load=None,state=None):
	...
	while (epoch <= max_epochs):
		...
		for step in range(1, train_X.shape[0]/batch_size + 1): # train for (train_X.shape[0]/batch_size) steps
			...
			xs,ys = get_aug_data(xs,ys) # get augmented batch data
			sess.run(train_step, feed_dict = {x:xs,y_:ys,isTraining:True,keep_prob1:prob1,keep_prob2:prob2}) # do training
			if step % show == 0 or step == (train_X.shape[0]/batch_size): # after every 100 steps show batch loss, acc
				data = 'batch_train'
				loss = sess.run(cross_entropy,feed_dict = {x:xs,y_:ys,isTraining:True,keep_prob1:1,keep_prob2:1})
				pred = sess.run(y,feed_dict = {x:xs,isTraining:True,keep_prob1:1,keep_prob2:1})
				acc  = accuracy(pred.reshape(4*batch_size,10),ys)
...
train(train_X,train_y,valid_X,valid_y,test_X)